...
def get_arguments(args=None):
    ...
    # EXPERIMENT
    parser.add_argument('--experiment_mode', help='train/test/train-cval/eval', type=str, required=True, default="train")  
    parser.add_argument('--run_id', help='identifier string for current experiment', type=str, default="")
    ...
    parser.add_argument('--output_path', help='Where is the output folder?', type=str,
                        default="E:/python/GLoMOT/output/")
    parser.set_defaults(mot_sub_folder='mot_files')  

    # SEED
    parser.add_argument('--seed', help='Seed of the experiment', type=int, default=0)

    # DEVICE SPECS
    parser.add_argument('--cuda', help='Run on gpu', dest='cuda', action='store_true',default=True)
    parser.add_argument('--gpus', help='use GPU numbers', default=[0])
    ...
    # DETECTIONS
    parser.add_argument('--det_file', help='Detection file to use', type=str, default = "byte065")
    ...
    # PRETRAINED MODELS
    parser.add_argument('--save_cp', help='Save checkpoints', dest='save_cp', action='store_true',default=True)
    ...
    parser.add_argument('--feature_embedding_model_path', help='Reid model path to obtain features', type=str,
                        default='')
    parser.add_argument('--model_path', help='GLoMOT model path to perform tracking', type=str,
                         default="")
    ...
    # TRAINING
    parser.add_argument('--num_batch', help='Number of graphs per batch', type=int, default=5)
    parser.add_argument('--lr', help='Learning rate', type=float, default=0.00015)
    ...
    parser.add_argument('--augmentation', help='Perform data augmentation during training', dest='augmentation', action='store_true', default=False)
    parser.add_argument('--train_dataset_frame_overlap', help='Most frames to overlap for different datapoints', type=int, default=1)
    parser.add_argument('--start_eval', help='Epoch at which evaluation will start', type=int, default=3)
    ...
    parser.add_argument('--min_track_len', help='Shorter tracks will be removed', type=int, default=1)#
    ...
    parser.add_argument('--no_interpolate', help='Do not interpolate missing detections of trajectories', dest='no_interpolate', action='store_true',default=True)


    # GRAPH PARAMETERS
    parser.add_argument('--top_k_nns', help='Similarity of nodes to connect in a graph', type=int, default=5)
    parser.add_argument('--frames_per_graph', help='Total number of frames to process', type=int,
                        default=2)
    parser.add_argument('--frames_per_level', type=int, nargs='*',default=[2])
    parser.add_argument('--pruning_method', type=str, help='Pruning scheme', default=['geometry'])
    parser.add_argument('--symmetric_edges', type=bool, default=True)  # eval:True  train:False

    # HIERARCHICAL PARAMETERS
    parser.add_argument('--hicl_depth', type=int, default=1)
    parser.add_argument('--node_id_min_ratio', type=float, default=0.5)
    parser.add_argument('--depth_pretrain_iteration', type=int, default=1000)

    # MOTION SETTING
    parser.add_argument('--motion_max_length', type=int, nargs='*',
                        help='Maximum number of frames used to encode trajectories before pred at each layer',
                        default=[])
    parser.add_argument('--motion_pred_length', type=int, nargs='*',
                        help='Number of future/past frames for which locations are predicted at each level',
                        default=[])
    parser.add_argument('--interpolate_motion', action='store_true',
                        help='If true, missing locations in each node location are filled via linear interpolation',
                        default=True)
    parser.add_argument('--linear_center_only', action='store_true',help='If true, the linear motion model will only predict center locations, and leave width/height constant', default=True)
    ...
    parser.add_argument('--zero_nodes', action='store_true', default=True)

    # FEATURE USAGE
    parser.add_argument('--mpn_use_motion', type=_store_bool, nargs='*',
                        help='Use motion predictions GIoU as edge feature at each layer',
                        default=[False])
    parser.add_argument('--mpn_use_reid_edge', type=_store_bool, nargs='*', default=[True])
    parser.add_argument('--mpn_use_pos_edge', type=_store_bool, nargs='*', default=[True])

    # VERBOSE
    parser.set_defaults(verbose_iteration=50)

    # EXPERIMENT EVAL
    parser.add_argument('--old_experiments', type=str, nargs='*', help='Experiment files to merge', default=[None])

    # LOSS FUNCTION
    parser.add_argument('--use_appearance_loss', type=str, default=False)
    parser.add_argument('--appearance_loss_weight', type=float, default=0.05)
    parser.add_argument('--use_temporal_loss', type=str, default=False)
    parser.add_argument('--depth_loss_weight', type=float, default=0.02)
    parser.add_argument('--feet_loss_weight', type=float, default=0.01)
    parser.add_argument('--script_appearance_loss', type=str, default=False)

    # NODE BUFF POOL
    parser.add_argument('--use_node_buffer',type=bool,default=True)
    parser.add_argument('--buffer_len', type=int, default=20)
    parser.add_argument('--use_depth', type=str,default=True)
    parser.add_argument('--use_buffer_motion', type=bool, default=False)
    parser.add_argument('--use_kal_or_diff_motion', type=bool, default=False)
    parser.add_argument('--script_kal_diff_motion', type=str, default=None) #yes:kal  no:diff
    parser.add_argument('--motion_decay_factor', type=float, default=0.9)
    parser.add_argument('--script_motion', type=str, default=None)
    parser.add_argument('--use_kalman_filter', type=bool, default=False)
    parser.add_argument('--encoder_config', default='')
    parser.add_argument("--encoder_path", default="", type=str, help="encoder model path")
    parser.add_argument('--use_sbs_reid', type=str, default=True)

    # DATASET SELECT
    parser.add_argument('--train_data_percent', type=float, default=0.5)
    parser.add_argument('--dataset', type=str, default="mot17")
    parser.add_argument('--dataset_type', type=str, default="val")
    parser.add_argument('--use_smpnet', type=bool, default=False)
...
def change_dataset_config(config):
    if config.dataset=="mot17":
        config.encoder_config='E:/python/GLOMOT/configs/diff_motion_config/mot_test.yaml'
        config.encoder_path="E:/python/GLOMOT/models/diff_motion/mot17_motion.pt"
    elif config.dataset=="mot20":
        config.encoder_config='E:/python/GLOMOT/configs/diff_motion_config/mot20_test.yaml'
        config.encoder_path="E:/python/GLOMOT/models/diff_motion/mot20_motion.pt"
    elif config.dataset=="dancetrack":
        config.encoder_config='E:/python/GLOMOT/configs/diff_motion_config/dancetrack_test.yaml'
        config.encoder_path="E:/python/GLOMOT/models/diff_motion/dancetrack_motion.pt"
    elif config.dataset=="sportsmot":
        config.encoder_config='E:/python/GLOMOT/configs/diff_motion_config/sportsmot_test.yaml'
        config.encoder_path="E:/python/GLOMOT/models/diff_motion/sportsmot_motion.pt"
    elif config.dataset=="visdrone":
        config.encoder_config='E:/python/GLOMOT/configs/diff_motion_config/visdrone_test.yaml'
        config.encoder_path="E:/python/GLOMOT/models/diff_motion/visdrone_motion.pt"
    else:
...
def create_experiment_path(config):
    ...
    """
    date = '{date:%m-%d_%H-%M-%S}'.format(date=datetime.now())
