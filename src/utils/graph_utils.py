...
def find_graph_time_valid_edges(node_frames, node_frames_mask, depth, frames_per_level, connectivity, pruning_method=['geometry', 'reid', 'reid']):
    ...
    edge_ixs = find_time_valid_connections(node_frames=node_frames)
...
def find_time_valid_connections(node_frames):
    """
    Return all possible connections between nodes in a first frame and a second frame.
    MODIFIED to be fully vectorized, avoiding slow Python loops.
    """
    # Identify nodes belonging to the first and second frames in the window
    first_frame_val = node_frames[0, 0]
    first_frame_mask = node_frames[:, 0] == first_frame_val

    # It's safer to not assume only two unique frames. This handles any number of frames.
    # We connect nodes from earlier frames to nodes in later frames.
    row, col = torch.where(node_frames < node_frames.T)

    # Ensure edges are between different nodes
    mask = row != col
    row, col = row[mask], col[mask]

    # Stack them into the required [2, num_edges] format
    edge_ixs = torch.stack([row, col], dim=0)

    # Sanity check that we are not creating edges within the same frame
    assert torch.all(node_frames[edge_ixs[0]] < node_frames[edge_ixs[1]]), \
        "Invalid edge created within the same frame or backwards in time."
...
def find_nearest_neighbor_connections(edge_ixs, pwise_dist, top_k_nns, reciprocal_k_nns=True, num_nodes=None):
    ...
    if edge_ixs.numel():
        ...
        if pwise_dist.dim() == 0:
            pwise_dist = pwise_dist.unsqueeze(0)
        ...
        # Select the topknn edges   [2,num_selected_edges]
...
def compute_pruning_score(x_reid, x_frame, x_feet, edge_ixs, pruning_method = 'geometry', graph_pruning_score=None,
    ...
    """
    #'geometry'返回脚点的几何距离分数(L2) 'reid'返回ReID特征的几何距离分数(L2) 'lpc'
    SIGMA_T = 40
    SIGMA_P = 100
    SIGMA_APP = 20 #决定了app_dist的归一化尺度，进而控制外观相似度(ReID)的衰减速度 越大衰减越慢

    assert pruning_method in ('geometry', 'reid', 'lpc', 'geometry_reid') or pruning_method.startswith(
        'motion_') or pruning_method.startswith('geometry_reid_')

    if pruning_method.startswith('motion_'):  # Motion
        # we expect something like # 'motion_01', where the second number indicates the weight
        # used to combine motion and reid    motion005-> 0.05 motion + 0.95 ReID
        ...
        return 2 - (weight * graph_pruning_score + (1 - weight) * torch.exp(-app_dist / SIGMA_APP))  # 指数归一化距离 返回值越小越相似


    if pruning_method.startswith('geometry_reid_'):  # mix position & appearance
        weight = pruning_method.split('_')[-1]
        assert weight.isdigit()
        import math
        weight = float(weight) * math.pow(10, -weight.count('0'))  # e.g. '001' --> '0.01'
        ...
        return 1 - (weight * torch.exp(-feet_dist / SIGMA_P) + (1 - weight) * torch.exp(-app_dist / SIGMA_APP))
...
def compute_edge_features(edge_ixs, node_frames, node_bboxes, node_feets, node_reids, fps, motion_feats={},
                          edge_feats_to_use=(
                                             ...
                                             reid_sim_fn=F.pairwise_distance,node_depth=None):
    """
    ...
    if edge_ixs.numel():
        ...
        node_depth = node_depth.to('cuda')

        # Get values per node  将时间帧转换为秒
        ...
        if node_depth.numel() > 0 :
            for i in range(0, edge_ixs.shape[1], 50000):
                src_ixs = node_depth[edge_ixs[0][i:i + 50000].long()]
                dst_ixs = node_depth[edge_ixs[1][i:i + 50000].long()]
                depth_diff = torch.abs(src_ixs - dst_ixs)
                depth_dists.append(depth_diff.view(-1, 1))
            depth_dists = torch.cat(depth_dists, dim=0).squeeze(1)
        ...
        edge_feats_dict = {'relative_x_vels': feet_vel_x[col] - feet_vel_x[row],
                           ...
                           'emb_dists': emb_dists,
                           'depth_dists':depth_dists,
                           'node_depth':node_depth,
                           'node_feets':node_feets[0],
                           'emb_cos_dists':emb_cos_dists}
    ...
    return edge_feats,edge_feats_dict
...
def assign_edge_labels(node_ids, edge_ixs, node_frames):
    ...
    original_node_ixs = torch.cat((same_id_edges[1][future_mask], torch.as_tensor([-1]).to(node_ids.device)))
    ...
    # Recover the ixs of active edges in the original edge_index tensor
...
def to_positive_decision_graph(mot_graph, threshold):
    ...
    mot_graph.edge_index = mot_graph.edge_index.T[edges_mask].T     #只保留正样本
