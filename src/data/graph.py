...
class HierarchicalGraph(Data):
    ...
    def _one_hot_encode_frames(self):
        ...
        num_class = 8
        return F.one_hot(norm_frames, num_classes=num_class)
    ...
    def construct_curr_graph_nodes(self, config):
        ...
        curr_graph = Graph(x=x_node, edge_index=raw_edge_index, x_reid=x_reid,y_id=y_id,  
                            ...
                            x_frame_start=x_frame, x_frame_end=x_frame,
                            ...
                            pruning_score=torch.zeros(raw_edge_index.shape[1], device=x_node.device),
                            x_depth=x_depth,
                            **motion_features)
    ...
    def add_edges_to_curr_graph(self, config, curr_graph):
        ...
        x_depth=curr_graph.x_depth
        
        # Prune time-valid edges
        #reid_sim_fn = F.pairwise_distance if config.reid_sim_fn == 'l2' else lambda x, y: 2 - F.cosine_similarity(x, y)
        reid_sim_fn = F.pairwise_distance if config.reid_sim_fn == 'l2' else lambda x, y: 1 - (F.cosine_similarity(x, y) + 1) / 2
        ...
        #edge_feats_to_use_ = ['depth_dists']
        ...
        edge_features,edge_feats_dict = compute_edge_features(edge_ixs=edge_ixs, node_frames=x_frame, node_bboxes=x_bbox,
                                              node_feets=x_feet, node_reids=x_reid, fps=self.fps,
                                              motion_feats=motion_feats,
                                              edge_feats_to_use = edge_feats_to_use_,
                                              reid_sim_fn=reid_sim_fn,node_depth=x_depth)

        reid_dist = edge_feats_dict['emb_dists']
        ...
        curr_graph = Graph(x=curr_graph.x, edge_attr=edge_features, edge_index=edge_ixs, edge_labels=edge_labels,
                           ...
                           edge_preds=edge_preds, x_reid=x_reid, edge_mask=edge_mask,x_depth=x_depth,reid_dist=edge_feats_dict['emb_dists'],x_feets=edge_feats_dict['node_feets'],depth_dists=edge_feats_dict['depth_dists'])

        self.edge_feats_dict = edge_feats_dict
        return curr_graph
