...
class ContextAwareGatingModule(nn.Module):
    def __init__(self, group_dims=[1, 2, 2, 1], hidden_dim=32):
    ...
    def forward(self, edge_attr, step=None):

        splits = torch.split(edge_attr, self.group_dims, dim=1)
        f_depth, f_pos, f_size, f_emb = splits  # f_pos: [B,2], f_emb: [B,1]
        spatial_dist = torch.norm(f_pos, dim=1)
        if spatial_dist.numel() > 1 and spatial_dist.std(unbiased=False) > 1e-6:
            spatial_dist_n = torch.sigmoid(
                (spatial_dist - spatial_dist.mean()) / (spatial_dist.std(unbiased=False) + 1e-6))
        else:
            spatial_dist_n = torch.sigmoid(spatial_dist)

        appearance_diff = f_emb.squeeze(1)
        if appearance_diff.numel() > 1 and appearance_diff.std(unbiased=False) > 1e-6:
            appearance_diff_n = torch.sigmoid(
                (appearance_diff - appearance_diff.mean()) / (appearance_diff.std(unbiased=False) + 1e-6))
        else:
            appearance_diff_n = torch.sigmoid(appearance_diff)

        context = torch.stack([spatial_dist_n, appearance_diff_n], dim=1)

        raw_gates = self.context_net(context)
        gates = F.softmax(raw_gates / self.temperature, dim=1)

        g_pos, g_emb = gates[:, 0].unsqueeze(1), gates[:, 1].unsqueeze(1)
        f_pos = f_pos * g_pos
        f_emb = f_emb * g_emb
        weighted_parts = torch.cat([f_depth, f_pos, f_size, f_emb], dim=1)


        return weighted_parts
    def compute_gate_alignment_loss(self, g_pos, g_emb, appearance_diff, spatial_dist):
        """
        Compute auxiliary loss to encourage expected gate behavior:
        - When appearance_diff is small → g_pos should be large
        - When spatial_dist is small → g_emb should be large
        """
        loss_pos = F.mse_loss(g_pos.squeeze(), 1.0 - appearance_diff.detach())
        loss_emb = F.mse_loss(g_emb.squeeze(), 1.0 - spatial_dist.detach())
        return loss_pos + loss_emb
...
class EdgeModel(nn.Module):
    ...
    """
    def __init__(self, edge_mlp,gating_module=None):
...
class TimeAwareNodeModel_with_attention(nn.Module):
    def __init__(self, flow_in_mlp, flow_out_mlp, node_mlp, node_agg_fn):
        ...
        self.attention = nn.Linear(2 * flow_in_mlp.fc_dims[-1], 1)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        flow_combined = torch.cat([flow_in, flow_out], dim=1)
        attn_scores = torch.sigmoid(self.attention(flow_combined))
        flow_in = flow_in * attn_scores
        flow_out = flow_out * attn_scores
...
class SMOTMPNet(nn.Module):

    def __init__(self, model_params, bb_encoder = None, motion=None, pos_feats=None, reid=None):

        super(SMOTMPNet, self).__init__()

        self.node_cnn = bb_encoder
        self.model_params = model_params

        self.feature_gating = ContextAwareGatingModule(
            group_dims=[1, 2, 2, 1],  # 根据原始特征分组
            hidden_dim=32
        )
    ...
    def forward(self, data, edge_level_embed=None, node_level_embed=None):
        ...
        edge_attr= self.feature_gating(edge_attr)
