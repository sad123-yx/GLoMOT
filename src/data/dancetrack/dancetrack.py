...
def get_dancetrack_det_df_from_det(seq_name, data_root_path, config):
    """
    ...
    elif config.det_file == 'byte045':
        det_df = det_df[det_df['conf'].ge(0.45)].copy()
    elif config.det_file == 'byte060':
        det_df = det_df[det_df['conf'].ge(0.60)].copy()
    ...
    det_df['frame_path'] = det_df['frame'].apply(lambda frame_num: osp.join(seq_path, f'img1/{int(frame_num):08}.jpg'))
...
def get_dancetrack_gt(seq_name, data_root_path, config):
    """
