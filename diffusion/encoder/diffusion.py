...
class D2MP_OB(Module):
    ...
    def sample(self, context, sample, bestof, point_dim=4, flexibility=0.0, ret_traj=False):
        ...
        for i in range(sample):
            batch_size = context.size(0) #box_num
            if bestof:
            ...
            cur_time = torch.ones((batch_size,), device=x_T.device)#[box_num]
            ...
            for t in range(self.var_sched.num_steps, 0, -1):
                ...
                x_t = traj[t]
                beta = cur_time.log() / 4
                t_tmp = cur_time.reshape(-1, 1)                    #[box_num,1]
                pred = self.net(x_t, beta=beta, context=context)   #[box_num,4]
                C_pred = pred
                noise_pred = (x_t - (t_tmp - 1) * C_pred) / t_tmp.sqrt() #[box_num,4]

                x0 = self.pred_x0_from_xt(x_t, noise_pred, C_pred, cur_time) #[box_num,4]
                x0.clamp_(-1., 1.)
                C_pred = -1 * x0
