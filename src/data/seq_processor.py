...
from src.data.sportsmot.sportsmot import get_sportsmot_gt, get_sportsmot_det_df_from_det,get_sportsmot_testset_det_df_from_det
...
_SEQ_TYPE_DETS_DF_LOADER = {'MOT': get_mot_det_df_from_det, 'MOT_GT': get_mot_det_df_from_gt, 'MOT_DANCETRACK': get_dancetrack_det_df_from_det,'MOT_SPORTSMOT': get_sportsmot_det_df_from_det,'MOT_SPORTSMOT_TEST': get_sportsmot_testset_det_df_from_det}
_SEQ_TYPE_GT_DF_LOADER = {'MOT': get_mot_gt, 'MOT_GT': get_mot_gt, 'MOT_DANCETRACK': get_dancetrack_gt, 'MOT_SPORTSMOT': get_sportsmot_gt,'MOT_SPORTSMOT_TEST': get_sportsmot_gt}
...
mot17_seqs += [f'MOT17-{seq_num:02}-{"FRCNN-N-2"}' for seq_num in (2, 4, 5, 9, 10, 11, 13)]
mot17_seqs += [f'MOT17-{seq_num:02}-{"FRCNN-N-5"}' for seq_num in (2, 4, 5, 9, 10, 11, 13)]
mot17_seqs += [f'MOT17-{seq_num:02}-{"FRCNN-N-10"}' for seq_num in (2, 4, 5, 9, 10, 11, 13)]
...
dancetrack_seqs  += [f'dancetrack{seq_num:04}-{"N-2"}' for seq_num in range(1,101)]
dancetrack_seqs  += [f'dancetrack{seq_num:04}-{"N-5"}' for seq_num in range(1,101)]
dancetrack_seqs  += [f'dancetrack{seq_num:04}-{"N-10"}' for seq_num in range(1,101)]
for seq_name in dancetrack_seqs:
    _SEQ_TYPES[seq_name] = 'MOT_DANCETRACK'

# -------SportsMOT------------
sportsmot_train_folder="E:/python/DiffusionTrack-AAAI/datasets/sportsmot/train"
sportsmot_trainval_folder = "E:/python/DiffusionTrack-AAAI/datasets/sportsmot/train_val"
sportsmot_val_folder = "E:/python/DiffusionTrack-AAAI/datasets/sportsmot/val"
sportsmot_test_folder = "E:/python/DiffusionTrack-AAAI/datasets/sportsmot/test"
...
sportsmot_trainset_seqs=sportsmot_train_sequence_list + sportsmot_val_sequence_list

for seq_name in sportsmot_trainset_seqs:
    _SEQ_TYPES[seq_name] = 'MOT_SPORTSMOT'
for seq_name in sportsmot_test_sequence_list:
    _SEQ_TYPES[seq_name] = 'MOT_SPORTSMOT_TEST'
...
class MOTSeqProcessor:
    ...
    def _store_dfs(self):
        ...
        # File names
        if self.config.det_thresh is not None:
            det_file_name="det0"+str(int(self.config.det_thresh*100))+".pkl"
            det_df_path = osp.join(processed_dets_path, det_file_name)
        else:
    ...
    def _store_embeddings(self):
        ...
        if osp.exists(node_embeds_path) and osp.exists(reid_embeds_path):
            print(
                f"Embeddings already exist at {node_embeds_path} and {reid_embeds_path}. Skipping embedding generation.")
            return
        ...
        for frame_start, frame_end in zip(frame_cutpoints[:-1], frame_cutpoints[1:]):
            ...
            bbox_loader = DataLoader(bbox_dataset, batch_size=50, pin_memory=False,  # 1000   True
    ...
    def _store_embeddings_with_sbs(self):

        assert self.feature_embedding_model is not None
        assert self.config.reid_embeddings_dir is not None and self.config.node_embeddings_dir

        if self.config.det_thresh is not None:
            det_file="det0"+str(int(self.config.det_thresh*100))
        else:
            det_file=self.config.det_file
        # Directory paths
        node_embeds_path = osp.join(self.det_df.seq_info_dict['seq_path'], 'processed_data/embeddings',
                                    det_file, self.config.node_embeddings_dir)

        reid_embeds_path = osp.join(self.det_df.seq_info_dict['seq_path'], 'processed_data/embeddings',
                                    det_file, self.config.reid_embeddings_dir)
        ...
        # Make sure that we don't run out of memory, so batch the detections if necessary
        num_dets = self.det_df.shape[0]
        max_dets_per_df = int(1e6)
        frame_cutpoints = [self.det_df.frame.iloc[i] for i in np.arange(0, num_dets, max_dets_per_df, dtype=int)]
        frame_cutpoints += [self.det_df.frame.iloc[-1] + 1]

        if self.config.dataset=="mot17":
            config_file="E:/python/GLoMOT/fast_reid/configs/MOT17/sbs_S50.yml"
            weights_path = "E:/python/GLoMOT/pretrained/mot17_sbs_S50.pth"
        elif self.config.dataset=="mot20":
            config_file="E:/python/GLoMOT/fast_reid/configs/MOT20/sbs_S50.yml"
            weights_path = "E:/python/GLoMOT/pretrained/mot20_sbs_S50.pth"
        elif self.config.dataset=="dancetrack":
            config_file="E:/python/GLoMOT/fast_reid/configs/Dancetrack/sbs_S50.yml"
            weights_path = "E:/python/GLoMOT/pretrained/dancetrack_sbs_S50.pth"
        elif self.config.dataset=="sportsmot":
            config_file="E:/python/GLoMOT/fast_reid/configs/SportsMOT/sbs_S50.yml"
            weights_path = "E:/python/GLoMOT/pretrained/sportsmot_sbs_S50.pth"

        ###
        print("config_file:{}, weights_path:{}".format(config_file, weights_path))
        ###

        fastreid_interface = FastReIDInterface(
        ...
        for frame_start, frame_end in zip(frame_cutpoints[:-1], frame_cutpoints[1:]):
            ...
            for frame in sub_df.frame.unique():
                ...
                frame_df = sub_df[sub_df.frame == frame]
                boxes = frame_df[['bb_left', 'bb_top', 'bb_right', 'bb_bot']].values  # Extract bounding boxes
                det_ids_frame = frame_df['detection_id'].values  # Extract detection IDs

                # Load the raw image for the current frame
                frame_path = frame_df['frame_path'].values[0]
                raw_img = cv2.imread(frame_path)  # Load image as BGR

                # Extract feature embeddings using FastReIDInterface.inference
                feature_emb = fastreid_interface.inference(raw_img, boxes)  # Shape: (num_detections, 2048)

                if feature_emb is None or len(feature_emb) == 0:
                    continue

                det_ids_tensor = torch.tensor(det_ids_frame, dtype=torch.float64).view(-1, 1)
                feature_tensor = torch.from_numpy(feature_emb)  # 默认 float64
                full_tensor = torch.cat((det_ids_tensor, feature_tensor), dim=1)

                # 保存当前帧的特征
                frame_node_embeds_path = osp.join(node_embeds_path, f"{frame}.pt")
                frame_reid_embeds_path = osp.join(reid_embeds_path, f"{frame}.pt")
                torch.save(full_tensor, frame_node_embeds_path)
                torch.save(full_tensor, frame_reid_embeds_path)

                seq_name=self.det_df.seq_info_dict['seq_path'].split("\\")[-1]
                print(f" {seq_name}: Frame {frame} embeddings saved.")
    ...
    def process_detections(self):
        ...
        self._assign_gt()  # Assign ground truth ids   遍历每一帧检测数据 在检测结果中为每个检测分配一个GT ID，以实现对真实目标的标识
        self._store_dfs()  # Store the detection and gt dframes
        if self.config.use_sbs_reid:
            self._store_embeddings_with_sbs()
        else:
    ...
    def _is_dets_and_embeds_ok(self, seq_path, seq_det_df_path):
        ...
        node_embeds_path = osp.join(seq_path, 'processed_data/embeddings',det_file,
                                    self.config.node_embeddings_dir)
        reid_embeds_path = osp.join(seq_path, 'processed_data/embeddings', det_file,
    ...
    def load_or_process_detections(self):
        ...
        if self.config.dataset=='sportsmot':
            if self.config.dataset_type=="test":
                seq_det_df_path = osp.join(seq_path, 'det', self.config.det_file + '.txt')
            else:
                seq_det_df_path = osp.join(seq_path, 'gt', 'gt.txt')
        else:
            seq_det_df_path = osp.join(seq_path, 'det', self.config.det_file + '.txt')
