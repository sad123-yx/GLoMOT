...
import diffusion.encoder.diffusion as diffusion
import numpy as np
from diffusion.encoder.diffusion import VarianceSchedule,D2MP_OB
...
class D2MP(Module):
    def __init__(self, config, encoder=None, device="cuda"):
        ...
        self.diffnet = getattr(diffusion, self.config.diffnet)
    ...
    def forward(self, batch):
        cond_encoded = self.encoder(batch["condition"])
